...
import getClassReport from "@salesforce/apex/Students.getClassReport";
...
export default class ClassProgress extends LightningElement {
	...
	wired_getClassReport(result) {
		...
		if (data) {
			...
			Utils.showNotification(this, { title: "Error (Instructor)", message: "Error getting class report", variant: Utils.variants.error });
			console.log(error);
			this.loading = false;
		}
	}
